...
import { Sliders, Music, Download, Moon, Sun, Trash2, RotateCcw, Heart, History, Check, AlertTriangle, Share2, Globe } from 'lucide-react';
import { useAudio } from '../context/AudioContext';
import { useTheme } from '../context/ThemeContext';
import { useLibrary } from '../context/LibraryContext';
import { playPopSfx } from '../services/soundEffects';
import { getShareWorkerUrl, setShareWorkerUrl } from '../services/shareService';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-16 select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* 4. CLOUDFLARE SHARE WORKER SETTINGS */}
        <ShareWorkerSettingCard showToast={showToast} />
...
function ShareWorkerSettingCard({ showToast }) {
  const [workerInput, setWorkerInput] = useState(() => getShareWorkerUrl());
  const [savedUrl, setSavedUrl] = useState(() => getShareWorkerUrl());

  const handleSave = (e) => {
    e.preventDefault();
    playPopSfx();
    setShareWorkerUrl(workerInput);
    setSavedUrl(workerInput.trim().replace(/\/$/, ''));
    showToast(workerInput.trim() ? 'Cloudflare Share Worker URL saved!' : 'Worker URL cleared. Using default app URL.');
  };
  ...
  return (
    <div className="toon-box p-6 bg-[var(--bg-secondary)] border-3.5 border-[var(--border-color)] shadow-[5px_5px_0px_var(--shadow-color)] space-y-4">
      <div className="flex items-center gap-2.5">
        <div className="p-2 rounded-xl bg-cyan-400 border-2.5 border-[var(--border-color)]">
          <Share2 className="w-5 h-5 text-stone-900" strokeWidth={2.5} />
        </div>
        <div>
          <h3 className="font-black text-lg text-[var(--text-primary)] font-['Grandstander']">Social Preview Worker URL</h3>
          <p className="text-xs font-bold text-[var(--text-muted)]">Set custom Cloudflare Worker for WhatsApp / Twitter Open Graph song previews</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-3">
        <div className="relative">
          <input
            type="url"
            placeholder="https://your-toon-tunes-share.workers.dev"
            value={workerInput}
            onChange={(e) => setWorkerInput(e.target.value)}
            className="w-full pl-9 pr-24 py-2.5 rounded-2xl bg-[var(--bg-primary)] border-2.5 border-[var(--border-color)] shadow-[2.5px_2.5px_0px_var(--shadow-color)] font-bold text-xs text-[var(--text-primary)] outline-none"
          />
          <Globe className="w-4 h-4 text-[var(--text-muted)] absolute left-3 top-1/2 -translate-y-1/2" />
          <button
            type="submit"
            className="absolute right-1.5 top-1/2 -translate-y-1/2 px-3 py-1 rounded-xl bg-amber-400 dark:bg-pink-500 text-stone-900 dark:text-white font-black text-xs border border-black shadow-[1px_1px_0px_#000] hover:scale-105 active:scale-95 transition-transform"
          >
            SAVE
          </button>
        </div>

        <div className="p-3 rounded-xl bg-[var(--bg-primary)] border-2 border-[var(--border-color)] text-[11px] font-bold space-y-1">
          <p className="text-[var(--text-primary)] flex items-center justify-between">
            <span>Status:</span>
            <span className={savedUrl ? 'text-emerald-500 font-black' : 'text-amber-500 font-black'}>
              {savedUrl ? `Active (${savedUrl})` : 'Default (App Origin)'}
            </span>
          </p>
          <p className="text-[var(--text-muted)] leading-relaxed">
            Worker script code available in <code className="text-pink-500 bg-stone-200 dark:bg-slate-800 px-1 py-0.5 rounded">worker/toon-tunes-share-worker.js</code>. Deploy to Cloudflare Workers to render Open Graph metadata!
          </p>
        </div>
      </form>
    </div>
