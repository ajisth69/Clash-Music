...
import { uploadPlaylist } from '../services/playlistDb';
...
export default function LibraryView({ onSelectArtist, onSelectAlbum, initialPlaylistId }) {
  ...
  const handleSharePlaylist = async (playlist) => {
    ...
    try {
      ...
      const res = await shareItem({ type: 'playlist', id: shortId, name: playlist.name });
      if (res.success) {
    ...
    <div className="space-y-5 pb-8 select-none">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <motion.button
            ...
            className={`px-3 py-1.5 rounded-xl border-2 border-[var(--border-color)] font-extrabold text-xs flex items-center gap-1.5 transition-all ${
              activeTab === 'liked'
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="font-bold text-xs sm:text-sm text-[var(--text-muted)]">
              All your favorite cartoon tracks saved in one place.
            </p>
            {likedSongs.length > 0 && (
              <motion.button
                whileHover={{ scale: 1.03 }}
          ...
          {likedSongs.length === 0 ? (
            ...
            <div className="grid grid-cols-3 xs:grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8 gap-2 sm:gap-2.5">
              {likedSongs.map((song) => (
                <SongCard key={song.id} song={song} queueList={likedSongs} />
              ))}
            </div>
          )}
        </div>
      ...
      {activeTab === 'playlists' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <p className="font-bold text-xs sm:text-sm text-[var(--text-muted)]">
              Create and manage your custom cartoon mixes.
            </p>
                ...
                <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2.5 sm:gap-3">
                  ...
                  {selectedPlaylist.tracks.map((song) => (
                    <div key={song.id} className="relative group">
                      <SongCard song={song} queueList={selectedPlaylist.tracks} />
                      <button
        ...
        <div className="space-y-3">
          ...
          {history.length === 0 ? (
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2.5 sm:gap-3">
              ...
              {history.map((song) => (
                <SongCard key={`hist-page-${song.id}`} song={song} queueList={history} />
              ))}
            </div>
          )}
        </div>
