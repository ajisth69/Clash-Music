...
import { searchSongs, searchAlbums, searchArtists } from '../services/musicApi';
...
export default function SearchView({ query, setQuery, onSelectAlbum, onSelectArtist }) {
  ...
  return (
    <div className="space-y-5 pb-mobile-safe">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          {(activeFilter === 'all' || activeFilter === 'albums') && albums.length > 0 && (
            <section className="space-y-2.5">
              <h3 className="text-lg font-black text-[var(--text-primary)] flex items-center gap-2">
                <Disc className="w-4 h-4 text-cyan-500" strokeWidth={2.5} />
              ...
              <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2.5 sm:gap-3">
                {albums.map((album) => (
                  <div
                    ...
                    className="toon-box p-2 cursor-pointer group hover:-translate-y-0.5 hover:shadow-[2.5px_2.5px_0px_var(--shadow-color)] transition-all bg-[var(--bg-secondary)] rounded-xl flex flex-col justify-between"
                  >
                    <div className="aspect-square w-full rounded-lg overflow-hidden border border-black shadow-[1.5px_1.5px_0px_#000] mb-1.5 bg-stone-200 dark:bg-slate-800 shrink-0">
                      <img
                        src={album.image}
                        alt={album.name}
                    ...
                    <div className="space-y-0.5">
                      <h4 className="font-extrabold text-[11px] sm:text-xs text-[var(--text-primary)] truncate leading-tight">
                        {album.name}
                      </h4>
          ...
          {(activeFilter === 'all' || activeFilter === 'artists') && artists.length > 0 && (
            <section className="space-y-2.5">
              <h3 className="text-lg font-black text-[var(--text-primary)] flex items-center gap-2">
                <Mic2 className="w-4 h-4 text-emerald-500" strokeWidth={2.5} />
                Artists ({artists.length})
              </h3>
              <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-2.5 sm:gap-3">
                {artists.map((artist) => (
                  <div
                    key={artist.id}
                    onClick={() => onSelectArtist(artist.name)}
                    className="toon-box p-2 text-center cursor-pointer group hover:-translate-y-0.5 hover:shadow-[2px_2px_0px_var(--shadow-color)] transition-all bg-[var(--bg-secondary)] rounded-xl flex flex-col items-center justify-center"
                  >
                    <div className="w-12 h-12 sm:w-14 sm:h-14 mx-auto rounded-full overflow-hidden border border-black shadow-[1.5px_1.5px_0px_#000] mb-1.5 bg-amber-300 dark:bg-pink-500 group-hover:scale-105 transition-transform flex items-center justify-center shrink-0">
                      <img
                        src={artist.image}
                        alt={artist.name}
          ...
          {!isLoading && songs.length === 0 && albums.length === 0 && artists.length === 0 && (
            <div className="toon-box p-12 text-center space-y-4 max-w-md mx-auto my-8 bg-amber-100 dark:bg-slate-800">
              ...
              <h3 className="text-2xl font-black text-stone-900 dark:text-white">No music tracks found!</h3>
