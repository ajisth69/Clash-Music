...
export default function FloatingDownloadWidget() {
  ...
  return (
    <div className="fixed top-20 sm:top-24 right-4 sm:right-6 z-[55] pointer-events-auto select-none flex flex-col items-end">
      ...
      <motion.button
        ...
        className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-amber-400 dark:bg-pink-500 text-stone-900 dark:text-white border-3 sm:border-3.5 border-black shadow-[4px_4px_0px_#000] flex items-center justify-center cursor-pointer group"
        title="Background Download Progress"
      >
        {isCompleted ? (
          <CheckCircle className="w-6 h-6 sm:w-7 sm:h-7 text-emerald-700 dark:text-emerald-300 animate-bounce" strokeWidth={2.5} />
        ) : (
          <Download className="w-6 h-6 sm:w-7 sm:h-7 text-stone-900 dark:text-white animate-bounce" strokeWidth={2.5} />
        )}
      ...
      {/* Expanded Progress Popover (Expands below top icon button) */}
      <AnimatePresence>
        {showPopover && (
          <motion.div
            ...
            className="mt-3 w-72 sm:w-80 toon-box p-4 bg-[var(--bg-secondary)] border-3.5 border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] space-y-3"
          >
