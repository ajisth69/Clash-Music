...
import { getArtistFullData } from '../services/musicApi';
...
export default function ArtistDetailView({ artistName, onBack }) {
  ...
  useEffect(() => {
    ...
    getArtistFullData(artistName).then(data => {
      ...
      console.warn('Artist data fetch failed:', err);
  ...
  // If no official registered artist profile page is available
      ...
      <div className="space-y-6 pb-12 select-none">
        ...
        <div className="toon-box p-10 text-center max-w-md mx-auto bg-[var(--bg-secondary)] border-3 border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] space-y-4">
          ...
          <p className="font-extrabold text-xs text-[var(--text-muted)] leading-relaxed">
            "{artistName}" does not have an officially registered artist profile page.
          </p>
          <button
            onClick={onBack}
      ...
      <div className="space-y-3">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8 gap-2 sm:gap-2.5">
            {songs.map((song) => (
              <SongCard key={song.id} song={song} queueList={songs} onSelectArtist={(name) => {
                if (name !== artistName) {
                  // navigate to new artist
                  onBack();
                  setTimeout(() => {
                    window.dispatchEvent(new CustomEvent('selectArtist', { detail: name }));
                  }, 50);
                }
              }} />
            ))}
