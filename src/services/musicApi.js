...
export function getDisplayImage(item) {
  ...
  if (Array.isArray(item.image) && item.image.length > 0) {
    const best = item.image.find(i => i && i.quality === '500x500') || item.image[item.image.length - 1];
...
export async function getArtistFullData(artistName) {
  ...
  try {
    ...
    const songsList = Array.from(songMap.values());

    // Flexible artist matching: exact match, substring match, or first search hit
    const exactMatch = Array.isArray(searchRes) ? (
      searchRes.find(r => normalizeArtistKey(r.name) === normKey && r.image) ||
      searchRes.find(r => normalizeArtistKey(r.name).includes(normKey) || normKey.includes(normalizeArtistKey(r.name))) ||
      searchRes[0]
    ) : null;

    const artistInfo = {
      id: exactMatch?.id || '',
      name: cleanText(exactMatch?.name || artistName),
      image: exactMatch?.image || songsList[0]?.image || null,
      role: exactMatch?.role || 'Artist',
      isVerified: songsList.length > 0 || !!exactMatch
    };
  ...
  try {
    ...
    if (isNaN(albumNameOrId) && typeof albumNameOrId === 'string') {
      ...
      const exactMatch =
        ...
        searchRes.find((r) => {
          const resKey = r.name.toLowerCase().trim().replace(/[^a-z0-9]/g, '');
          return resKey.includes(key) || key.includes(resKey);
        }) ||
        searchRes[0];
      ...
      if (exactMatch) {
        ...
        const invalidResult = {
          albumInfo: { id: '', name: albumNameOrId, isVerified: true },
          songs: []
        };
